...
def test_save_load_round_trip(tmp_path: Path, monkeypatch):
    ...
    original = Config()
    original.vast_api_key = "round-trip-key"
    ...
    assert loaded.vast_api_key == "round-trip-key"
...
def test_resolve_api_key_env_override(tmp_path: Path, monkeypatch):
    ...
    assert config.vast_api_key == "env-key"
...
def test_save_config_all_fields(tmp_path: Path):
    ...
    config.vast_api_key = "all-fields-key"
    ...
    assert data["vast_api_key"] == "all-fields-key"
...
def test_save_config_atomic_write_failure(tmp_path: Path):
    ...
    config = Config()
    config._config_path = cfg_path
    config.vast_api_key = "should-not-persist"
