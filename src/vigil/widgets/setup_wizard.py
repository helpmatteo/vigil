...
class SetupWizardScreen(Screen[bool]):
    ...
    def __init__(
        self, config: Config, http_client: httpx.AsyncClient | None = None, provider: Provider | None = None
    ) -> None:
        super().__init__()
        self.config = config
        ...
        if provider is None:
            ...
            provider = get_provider(getattr(config, "provider", "vast"))
    ...
    def _render_api_key_step(self, content: Vertical) -> None:
        ...
        desc.update(
            f"Real-time TUI for monitoring {self.provider.display_name} GPU training instances.\n"
            "Press S anytime to return to this wizard."
        )

        # Pre-fill from existing config
        existing = self.config.api_key or ""
        source = ""
        if existing:
            import os

            env_vars = ["VIGIL_API_KEY", *self.provider.env_var_names()]
            source = "Found from env var" if any(os.environ.get(v) for v in env_vars) else "Found from config"

        content.mount(Label(f"{self.provider.display_name} API Key:"))
        content.mount(Input(
            value=existing,
    ...
    def _render_ssh_key_step(self, content: Vertical) -> None:
        ...
        desc = self.query_one(".step-description", Static)
        desc.update(f"Select the SSH key registered with your {self.provider.display_name} account.")
        ...
        content.mount(Static(f"[dim]Make sure this key is registered in your {provider_name} account settings.[/dim]"))
    ...
    async def _validate_api_key(self) -> None:
        ...
        try:
            from ..discovery import RateLimitError

            self.config.api_key = key
            result = await self.provider.fetch_instances(key, self._http_client)
            ...
            if exc.response.status_code == 401:
                status.update(
                    f"[red]✗ Invalid API key — check your {self.provider.display_name} dashboard[/red]"
                )
            else:
                status.update(f"[red]✗ API error: {exc.response.status_code}[/red]")
        except Exception:
            status.update(
                f"[yellow]⚠ Could not reach {self.provider.display_name} API — key saved, will retry on launch[/yellow]"
            )
            self._api_validated = True
