...
from ..discovery import InstanceInfo, destroy_instance
...
class InstanceManagerScreen(Screen):
    ...
    def action_destroy_stuck(self) -> None:
        ...
        inst = next((i for i in self.stuck_instances if i.id == instance_id), None)
        if inst is None:
            return
    ...
    async def _do_destroy(self, instance_id: int) -> None:
        try:
            await destroy_instance(self._api_key, instance_id, self._http_client)
            if self.is_attached:
