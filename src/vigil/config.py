...
class Config:
    ...
    instances: dict[int, InstanceConfig] = field(default_factory=dict)
    ...
    def log_command_for(self, instance_id: int) -> str:
        inst = self.instances.get(instance_id)
        if inst and inst.log_command:
            return inst.log_command
        return self.default_log_command

    def stall_threshold_for(self, instance_id: int) -> int:
        inst = self.instances.get(instance_id)
    ...
    def ssh_username_for(self, instance_id: int) -> str:
        inst = self.instances.get(instance_id)
    ...
    def save_config(self, path: Path | None = None) -> None:
        ...
        if self.vast_api_key and self.vast_api_key != defaults.vast_api_key:
            data["vast_api_key"] = self.vast_api_key
...
def _apply_yaml_fields(config: Config, data: dict) -> None:
    ...
    _set_if_present(
        config, data,
        ("vast_api_key", "default_log_command", "alert_webhook_url", "ssh_username", "alert_webhook_format"),
        str,
    )
    ...
    for iid, idata in (data.get("instances") or {}).items():
        if isinstance(idata, dict):
            config.instances[int(iid)] = _parse_instance(idata)
...
def _resolve_api_key(config: Config) -> None:
    """Apply env-var and file-based fallbacks for the API key."""
    env_key = os.environ.get("VAST_API_KEY")
    if env_key:
        config.vast_api_key = env_key
    if not config.vast_api_key and DEFAULT_API_KEY_PATH.exists():
        config.vast_api_key = DEFAULT_API_KEY_PATH.read_text().strip()
