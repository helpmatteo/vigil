...
class LogStorage:
    def __init__(self, base_dir: Path) -> None:
        ...
        self._files: dict[int | str, io.TextIOWrapper] = {}
        self._metric_files: dict[int | str, io.TextIOWrapper] = {}

    def _open_log(self, instance_id: int | str) -> io.TextIOWrapper:
    ...
    def _open_metrics(self, instance_id: int | str, timestamp_str: str) -> None:
        instance_dir = self.base_dir / str(instance_id)
        instance_dir.mkdir(parents=True, exist_ok=True)
    ...
    def write_line(self, instance_id: int | str, line: str) -> None:
        with self._lock:
    ...
    def write_metrics(
        self, instance_id: int | str, metrics: dict[str, str], timestamp: float
    ) -> None:
        with self._lock:
    ...
    def flush(self, instance_id: int | str | None = None) -> None:
        with self._lock:
    ...
    def close(self, instance_id: int | str | None = None) -> None:
        with self._lock:
            if instance_id is not None:
    ...
    def log_dir_for(self, instance_id: int | str) -> Path:
    ...
    def list_sessions(self, instance_id: int | str) -> list[Path]:
        instance_dir = self.base_dir / str(instance_id)
        if not instance_dir.exists():
            return []
        return sorted(instance_dir.glob("raw_*.log"), reverse=True)

    def list_metric_sessions(self, instance_id: int | str) -> list[Path]:
        instance_dir = self.base_dir / str(instance_id)
        if not instance_dir.exists():
            return []
    ...
    def cleanup(self, retention_days: int = 0, max_size_mb: int = 0) -> dict[str, int]:
        ...
        with self._lock:
            ...
            active_ids = {str(k) for k in self._files.keys()} | {str(k) for k in self._metric_files.keys()}
        ...
        if retention_days > 0:
            ...
            for instance_dir in self.base_dir.iterdir():
                ...
                try:
                    if instance_dir.name in active_ids:
                        continue
