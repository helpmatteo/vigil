...
from .discovery import InstanceInfo, RateLimitError, destroy_instance, fetch_credit, fetch_instances
...
class VastDashboard(App):
    """Real-time TUI dashboard for Vast.ai training instances."""

    TITLE = "Vast.ai Dashboard"
    ...
    def __init__(self, config: Config, state: AppState | None = None) -> None:
        ...
        self._panels: dict[int, InstancePanel] = {}
        self._stream_workers: dict[int, Worker] = {}
    ...
    def on_mount(self) -> None:
        ...
        if not self.app_config.vast_api_key:
            # First launch — push wizard before starting workers
            self.push_screen(
                SetupWizardScreen(self.app_config, self._http_client),
    ...
    def _on_wizard_complete(self, completed: bool) -> None:
        ...
        if self.app_config.vast_api_key:
            self._start_workers()
    ...
    async def _discover_loop(self) -> None:
        ...
        while True:
            try:
                result = await fetch_instances(
                    self.app_config.vast_api_key, self._http_client
                )
    ...
    def _restart_stream(self, instance_id: int) -> None:
    ...
    def action_destroy_instance(self) -> None:
        ...
        if self._focused_id is None and stuck_id is not None:
            inst = next((i for i in self._stuck_instances if i.id == stuck_id), None)
            if inst is None:
                return
    ...
    async def _do_destroy(self, instance_id: int) -> None:
        try:
